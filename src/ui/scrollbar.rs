...
impl Scrollbar {
    fn new(max: usize, pos: usize, orientatation: Orientation) -> Self {
        Self {
...
impl Widget for Scrollbar {
    fn render(self, area: Rect, buf: &mut Buffer) {
        match &self.orientatation {
            Orientation::Vertical => self.render_vertical(area, buf),
            Orientation::Horizontal => self.render_horizontal(area, buf),
        }
    }
}
...
pub fn draw_scrollbar<B: Backend>(
    ...
    let mut widget = Scrollbar::new(max, pos, orientatation);
